    ...
    <!-- ROW -->
      <div class="row">
        ...
        <section class="col-lg-12 connectedSortable">
          ...
          {!! Form::model($report, ['route' => ['reports.store'], 'class' => 'form-horizontal']) !!}
              ...
              <div class="form-group row">
                ...
                <div class="col-sm-10">
                  ...
                  <div class="all-shelter input-group input-group-lg mb-3">
                    ...
                    {{ Form::select('shelter_id', $shelters, null, ['id' => 'all_shelter_select', 'class' => 'form-control shelter_select', 'placeholder' => '-- 支援先を選択してください --']) }}
                  ...
                  {{-- 過去に入力した支援先 --}}
                  {{ Form::select('shelter_id', $prev_shelters, null, ['id' => 'prev_shelter_select', 'class' => 'form-control shelter_select select2', 'placeholder' => '-- 支援先を選択してください --']) }}
                  ...
                  <div class="row">
                    <div class="col">
                      <blockquote>
                        ...
                        →同日の活動内容<br>
                        <br>
                        S（subjective）：主観的情報<br>
                        → 対象者が話した内容などから得られた情報<br>
                        <br>
                        O（objective）： 客観的情報<br>
                        → 調査や検査からなど得られた客観的な情報<br>
                        <br>
                        A（assessment）： 評価<br>
                        → O と Sの内容を元に分析や解釈を行った総合的な評価<br>
                        <br>
                        P（plan）： 計画（支援計画）<br>
                        → Aに基づいて決定した支援の方針・内容など<br>

                        <button type="button" id="template-comment" class="btn btn-default">テンプレートをコピーする</button>
                      </blockquote>
                    </div>

                    <div class="col">
                      @if ($prev_report && filled($prev_report->comment))
                        <blockquote>
                          [前回の日報]<br>
                          {!! nl2br(e($prev_report->comment)) !!}
                          <br>
                          <button type="button" id="prev-comment" class="btn btn-default">入力欄にコピーする</button>
...
@section('script')
  ...
  <script>
    ...
    const templateComment = `A（action）：活動内容
→
...
→
    ...
    const prevComment = @json($prev_report ? $prev_report->comment : "");


    $("#prev-comment").on('click', function(){
      $('#comment').val(prevComment)
    });

    $("#template-comment").on('click', function(){
      $('#comment').val(templateComment)
    });
    ...
    $('#prev_writer_check').on('change', function() {
      ...
      <script>
        ...
        // 支援先が変更された場合はパラメータを変えて画面を再描画
        $(".shelter_select").on('change', function(event){

          var shelterId = $(event.target).val();

          var usePrevShelter = $("#prev_shelter_check").prop("checked");

          location.href = "{{ route('reports.create') }}?selected_shelter_id="+shelterId+"&usePrevShelter="+usePrevShelter;
        })
